...
**Languages which uses garbage collector :** Java, Python, C#, Go, JavaScript . Therefore are different types of garbage collection algorithm used . 
...
5. Pushing to stack is father than allocating on the heap , because we are always pushing on top of the stack

### Heap
1. Large and unordered pool of memory used for storing dynamically allocated data
2. When a program memory at run time , it requests space from the HEAP 
